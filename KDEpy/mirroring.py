...
def mirror_data(data, boundaries, pdf_values=None, decimals=10):
    ...
    def sum_together_np(mirrored_data, updated_values, decimals=10):
        ...
        summed_values = np.zeros(
            unique_data.shape[0]
        )  # Preallocate the summed values array
        np.add.at(
            summed_values, indices, updated_values
        ...
        if boundary is not None:
            ...
            if lower is not None:
                try:
                    ...
                    mirrored_points = np.column_stack(
                        [
                            lower_mirror if i == dim else data[:, i]
                    ...
                    mirrored_data, updated_values = sum_together_np(
                        mirrored_data, updated_values
                    )
                except:
                    pass
            if upper is not None:
                try:
                    ...
                    mirrored_points = np.column_stack(
                        [
                            upper_mirror if i == dim else data[:, i]
                            for i in range(data.shape[1])
                        ]
                    )
                    mirrored_data = np.vstack([mirrored_data, mirrored_points])
...
kernel_std = [
    silvermans_rule(data[:, column].reshape(-1, 1)) for column in range(data.shape[1])
]
...
plt.scatter(
    ...
    c="r",
    alpha=0.5,
    label="Bounded Mirrored resampling",
)
