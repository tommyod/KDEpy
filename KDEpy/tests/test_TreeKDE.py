...
def test_additivity_with_weights(data, split_index):
    ...
    # Fit to splits, and compensate for smaller data using weights
    y_1 = TreeKDE("epa").fit(data_first_split, weights_first_split).evaluate(x) * sum(weights_first_split)

    y_2 = TreeKDE("epa").fit(data_second_split, weights_second_split).evaluate(x) * sum(weights_second_split)
